  ...
  var postRequest = Https.request(postOptions, function (result) {
    if (result.statusCode == 200) {
      ...
      return;*/
          ...
          var postRequest1 = Https.request(postOptions1, function (result) {
            if (result.statusCode == 200) {
              callback(null, {
                result: 'SUCCESS'
              });
              ...
              console.error('Failed to send message to collider 1: ' + result.statusCode);
              callback(result.statusCode);
              return;
            }
          });

          postRequest1.write(message);
          postRequest1.end();
...
exports.main = {
  handler: function (request, reply) {
    ...
    rooms.get(key, function (error, room) {
      if (room) {
        console.log('main function :- Room ' + roomId + ' has state ' + room.toString());
...
exports.join = {
  handler: function (request, reply) {
    if(roomId==null)
    {
        roomId = Common.generateRandom(8);
    } 
    ...
    rooms.createIfNotExist(key, function (error, room) {
      ...
      if (occupancy >= 2) {
        ...
        var postRequest = Https.request(postOptions);
      }
    });
    ...
    addClientToRoom(request, roomId, clientId, isLoopback, function(error, result) {
      ...
      console.log('join function :- Room ' + roomId + ' has state ' + result.room_state);
    });
  }
};
...
exports.leave = {
  handler: function (request, reply) {
    ...
    removeClientFromRoom(request.headers['host'], request.params.roomId, clientId, function (error, result) {
      if (error) {
        console.log('leave function :- Room ' + request.params.roomId + ' has state ' + result.room_state);
      }
      console.log('leave function :- Room ' + request.params.roomId + ' has state ' + result.room_state);
      reply('hung up');
    });
    
    var key = Common.getCacheKeyForRoom(request.headers['host'], request.params.roomId);
    rooms.removeRooms(key);
    if(roomId==request.params.roomId)
    {
        var roomId = Common.generateRandom(8);
    }
  }
};
